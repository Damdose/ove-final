...
import { IT_SECTOR_CARDS } from "./it-landing-data";

const itTheme = POLE_THEMES.it;

const eyebrowSecteurs = `inline-flex items-center rounded-full px-3 py-1 font-sans text-xs font-semibold uppercase tracking-[0.14em] ${itTheme.primary.bgClass} ${itTheme.primary.textOnPrimaryClass}`;

const sectorPill = `inline-flex max-w-[min(100%,11rem)] items-center justify-center text-balance rounded-full px-3 py-1.5 text-center font-sans text-[0.65rem] font-semibold uppercase leading-snug tracking-[0.1em] shadow-[0_6px_18px_rgba(13,11,74,0.18)] ring-1 ring-white/40 sm:text-xs ${itTheme.primary.bgClass} ${itTheme.primary.textOnPrimaryClass}`;

export function ItLandingSectors() {
  return (
        ...
        <div className="mx-auto w-full max-w-screen-xl">
          <div className={SECTION_PAD}>
            <div className="mx-auto mb-12 max-w-3xl text-center sm:mb-14">
              ...
              >
                Une approche IT adaptée aux contraintes métiers : disponibilité, conformité et sécurité des données.
              </p>
            </div>
            <div className="mx-auto grid max-w-3xl grid-cols-1 gap-4 sm:max-w-4xl sm:grid-cols-2 sm:gap-4 lg:max-w-5xl lg:grid-cols-3">
              {IT_SECTOR_CARDS.map((card) => (
                <article
                  key={card.id}
