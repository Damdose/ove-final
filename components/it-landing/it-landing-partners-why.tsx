...
import { IT_WHY_CARDS } from "./it-landing-data";

const itTheme = POLE_THEMES.it;

const whyEyebrowClass = `inline-flex items-center rounded-full px-3 py-1 font-sans text-xs font-semibold uppercase tracking-[0.14em] ${itTheme.tag.bgClass} ${itTheme.tag.textClass}`;
...
export function ItLandingPartnersWhy() {
  return (
          ...
          <div className={SECTION_PAD}>
            <div className="mx-auto mb-12 max-w-3xl text-center sm:mb-16">
              ...
              <h2 className={`mt-4 font-display text-3xl font-bold leading-tight sm:text-4xl lg:text-5xl ${TEXT_INK}`}>
                Pourquoi choisir Ovedex IT ?
              </h2>
            </div>

            <div className="grid grid-cols-1 gap-6 sm:grid-cols-2 sm:gap-8 lg:grid-cols-3">
              {IT_WHY_CARDS.map((card, index) => (
                <div
                  key={index}
                ...
                >
                  <div
                    className={`flex h-14 w-14 shrink-0 items-center justify-center rounded-full ${BG_INK} shadow-[0_6px_20px_rgba(13,11,74,0.18)] ring-2 ring-purple-400/35 transition-[box-shadow,transform] duration-300 group-hover/card:scale-[1.03] group-hover/card:ring-purple-400/50`}
                  >
                    <WhyIcon />
                  </div>
