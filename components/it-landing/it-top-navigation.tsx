...
import { IT_LOGO_SRC, IT_SERVICE_NAV_TABS } from "./it-landing-data";
...
const RESOURCE_LINKS = [
  {
    href: "/it/newsroom",
    title: "Newsroom",
    description: "Articles cybersécurité, infogérance, cloud et bonnes pratiques du pôle IT.",
  },
  {
    href: "/it/cas-clients",
    title: "Cas clients",
    description: "Références SI, infogérance et projets critiques pour entreprises et ETI.",
  },
  {
    href: "/it/faq",
    title: "FAQ",
    description: "Réponses sur nos prestations IT, astreinte et modalités de collaboration.",
  },
] as const;
...
export function ItTopNavigation() {
  const baseId = useId();
  const servicesToggleId = `it-dropdown-toggle-0-${baseId}`;
  const servicesListId = `it-dropdown-list-0-${baseId}`;
  const resourcesToggleId = `it-dropdown-toggle-1-${baseId}`;
  const resourcesListId = `it-dropdown-list-1-${baseId}`;
  ...
  const tabIds = useMemo(
    () => IT_SERVICE_NAV_TABS.map((_, i) => ({ tab: `it-tabs-0-data-w-tab-${i}-${baseId}` })),
    [baseId],
  );
      ...
      <div className={`${PAGE_X} py-6 sm:py-8 lg:py-10`}>
        <div className="mx-auto max-w-screen-xl">
          <div className="grid w-full grid-cols-1 gap-10 gap-x-11 lg:grid-cols-[2.5fr_1fr]">
            <div>
              <div className="mb-5 inline-flex rounded-full bg-purple-400 px-3 py-1 font-sans text-xs font-semibold uppercase tracking-[0.14em] text-white">
                Nos services
              </div>
                  ...
                  {IT_SERVICE_NAV_TABS.map((tab, index) => {
                    ...
                    return (
                      ...
                      >
                        ...
                        <ServiceMegaTabChevron className="hidden text-purple-400/60 sm:inline-block" />
                      </Link>
                    );
                  })}
                </div>
                  ...
                  {IT_SERVICE_NAV_TABS.map((tab, index) => {
                    ...
                    return (
                      <div
                        ...
                        id={tab.id}
                        role="tabpanel"
                        aria-labelledby={index === 0 ? "tabelement-it" : tabIds[index]?.tab}
                      >
                            ...
                            {tab.links.map((link) => (
                              <li key={link.href} role="none">
                                <Link href={link.href} className={serviceCardClass} onClick={closeAll}>
                                  ...
                                  <ArrowRightIcon className="text-purple-400/45 transition group-hover:translate-x-0.5 group-hover:text-purple-500/80" />
                                </Link>
                              </li>
                            ))}
                          </ul>
            ...
            <div className="hidden lg:block">
              ...
              <div className="flex flex-col gap-3 font-display">
                <a
                  ...
                  className={`flex items-center justify-between gap-3 rounded-2xl border border-black/[0.08] bg-white px-4 py-3 no-underline transition-colors hover:border-purple-400/22 hover:bg-slate-50/80 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-offset-2 ${RING_INK_FOCUS}`}
                >
      ...
      >
        ...
        >
          <div className="flex flex-col gap-1 lg:flex-row lg:items-center lg:gap-1">
                ...
                <div className={`overflow-hidden bg-white ${R_CARD} ${BD_CARD} ${SHADOW_POP}`}>
                  <div className="border-b border-black/[0.06] bg-gradient-to-br from-slate-50/80 to-white px-4 py-3">
                    ...
                    <p className="m-0 mt-1 text-sm leading-snug text-slate-600">
                      Contenus utiles sur l’infogérance, la sécurité et le pilotage de votre SI.
                    </p>
                  </div>
                        ...
                        <Link href={item.href} className={resourceRowClass} onClick={closeAll}>
                          <span className="min-w-0 flex-1">
                            <span className="flex items-center justify-between gap-2">
                              ...
                              <ArrowRightIcon className="shrink-0 text-purple-400/50 transition group-hover:translate-x-0.5 group-hover:text-purple-500/85" />
                            </span>
            ...
            <Link href="/maintenance-it" className={`${navItem} w-full px-4 lg:w-auto lg:px-3`} onClick={closeAll}>
              Maintenance
            </Link>
            <Link
              href="/contact"
