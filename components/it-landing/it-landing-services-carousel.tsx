...
import { IT_CAROUSEL_SLIDES } from "./it-landing-data";
import { ItServiceCarouselIcon } from "./it-service-carousel-icon";
...
const badgeServices = `inline-flex items-center rounded-full px-3 py-1 font-sans text-xs font-semibold uppercase tracking-[0.14em] ${itTheme.primary.bgClass} ${itTheme.primary.textOnPrimaryClass}`;

function ServiceCardMediaBadge({ slug }: { slug: string }) {
  return (
    <span
      className={`pointer-events-none absolute left-4 top-4 z-10 flex h-10 w-10 items-center justify-center rounded-lg shadow-[0_6px_16px_rgba(13,11,74,0.12)] ring-1 ring-white/30 ${itTheme.primary.bgClass}`}
      aria-hidden
    >
      <ItServiceCarouselIcon slug={slug} className={`h-5 w-5 ${itTheme.primary.textOnPrimaryClass}`} />
    </span>
  );
}

export function ItLandingServicesCarousel() {
  const maskId = useId();
  const [slide, setSlide] = useState(0);
  const total = IT_CAROUSEL_SLIDES.length;
  ...
  return (
    <section className={`overflow-x-hidden ${SECTION_BG}`} aria-labelledby="it-services-carousel-heading">
              ...
              >
                ...
                >
                      ...
                      >
                        <span
                          className={`block rounded-full transition-colors duration-200 ${
                            i === slide ? "h-2 w-2 bg-purple-500" : "h-2 w-2 bg-neutral-300 hover:bg-neutral-400"
                          }`}
                          aria-hidden
                        />
                      </button>
                  ...
                  <button
                    ...
                    aria-controls={`w-slider-mask-it-${maskId}`}
