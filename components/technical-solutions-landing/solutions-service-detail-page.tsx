...
import type { SolutionsServiceDefinition } from "@/lib/solutions-services-catalog";
import { SOLUTIONS_SERVICE_FEATURES } from "@/lib/solutions-service-features";
...
function SolutionsServiceHero({ service }: { service: SolutionsServiceDefinition }) {
  return (
          ...
          <div className={`overflow-hidden bg-blue-200 ${R_CARD} ${BD_CARD} ${RING_SOFT} ${SHADOW_SURFACE}`}>
            ...
            >
              <div className="order-2 lg:order-1">
                <span
                  className={`inline-flex items-center rounded-full px-3 py-1 font-sans text-xs font-semibold uppercase tracking-[0.14em] ${solTheme.primary.bgClass} ${solTheme.primary.textOnPrimaryClass}`}
                >
                  Pôle Solutions
                </span>
                <h1
...
export function SolutionsServiceDetailPage({ service }: { service: SolutionsServiceDefinition }) {
  const features = SOLUTIONS_SERVICE_FEATURES[service.slug];
