...
import {
  SOLUTIONS_BLUR_GRADIENT_SRC,
  SOLUTIONS_HERO_ICON_SRC,
  SOLUTIONS_HERO_IMAGE_SRC,
} from "./technical-solutions-landing-content";

export function SolutionsMarketingHero() {
  return (
    <header className="relative max-sm:overflow-x-hidden max-sm:overflow-y-hidden max-sm:pt-0 pt-5">
      <div className="max-sm:px-[3%] px-[5%]">
        <div className="max-sm:max-w-full w-full max-w-screen-xl mx-auto max-sm:px-0 bg-blue-200 px-16 rounded-3xl">
              ...
              <div className="max-lg:min-h-[auto] grid grid-rows-[auto] auto-cols-[1fr] items-center max-sm:flex-col max-sm:flex gap-x-8 gap-y-8 grid-cols-[1.25fr_1fr]">
                <div>
                  ...
                  <p className="text-brand-ink font-sans text-lg font-light leading-7 mb-10 max-sm:pr-0 pr-10">
                    Conception, intégration et ingénierie en courants faibles : Ovedex Solutions
                    accompagne les entreprises, collectivités et structures exigeantes dans la mise en
                    place de systèmes techniques sûrs, fiables et pérennes.
                  </p>
                  <div>
      ...
      <Image
        src={SOLUTIONS_BLUR_GRADIENT_SRC}
        loading="lazy"
        alt=""
        width={800}
        height={400}
        className="inline-block max-sm:overflow-x-hidden max-sm:overflow-y-hidden max-sm:left-auto max-sm:top-[0%] z-[-1] absolute left-[12%] top-[-13%]"
      />
    </header>
  );
}
