...
import { SOLUTIONS_LOGO_SRC, SOLUTIONS_SERVICE_NAV_TABS } from "./data";
...
const RESOURCE_LINKS = [
  {
    href: "/solutions/newsroom",
    title: "Newsroom",
    description: "Articles techniques, sûreté, réseaux et conformité du pôle Solutions.",
  },
  {
    href: "/solutions/cas-clients",
    title: "Cas clients",
    description: "Références installations courants faibles et environnements sensibles.",
  },
  {
    href: "/solutions/faq",
    title: "FAQ",
    description: "Réponses sur nos prestations Solutions et modalités d’intervention.",
  },
] as const;
...
export function SolutionsTopNavigation() {
  const baseId = useId();
  const servicesToggleId = `sol-dropdown-toggle-0-${baseId}`;
  const servicesListId = `sol-dropdown-list-0-${baseId}`;
  const resourcesToggleId = `sol-dropdown-toggle-1-${baseId}`;
  const resourcesListId = `sol-dropdown-list-1-${baseId}`;
  ...
  const tabIds = useMemo(
    () => SOLUTIONS_SERVICE_NAV_TABS.map((_, i) => ({ tab: `sol-tabs-0-data-w-tab-${i}-${baseId}` })),
    [baseId],
  );
      ...
      <div className={`${PAGE_X} py-6 sm:py-8 lg:py-10`}>
        <div className="mx-auto max-w-screen-xl">
          <div className="grid w-full grid-cols-1 gap-10 gap-x-11 lg:grid-cols-[2.5fr_1fr]">
            <div>
              <div className="mb-5 inline-flex rounded-full bg-blue-400 px-3 py-1 font-sans text-xs font-semibold uppercase tracking-[0.14em] text-white">
                Nos services
              </div>
                    ...
                    return (
                      ...
                      >
                        <span className="leading-snug">
                          <span className={selected ? "font-semibold" : "font-medium"}>{tab.labelLines[0]}</span>
                          <br />
                          <span className={selected ? "font-semibold" : "font-medium"}>{tab.labelLines[1]}</span>
                        </span>
                        <ServiceMegaTabChevron className="hidden text-blue-400/55 sm:inline-block" />
                      </Link>
                    );
                  })}
                </div>
                  ...
                  {SOLUTIONS_SERVICE_NAV_TABS.map((tab, index) => {
                    ...
                    return (
                      <div
                        ...
                        id={tab.id}
                        role="tabpanel"
                        aria-labelledby={index === 0 ? "tabelement-sol" : tabIds[index]?.tab}
                      >
                            ...
                            {tab.links.map((link) => (
                              <li key={link.href} role="none">
                                <Link href={link.href} className={serviceCardClass} onClick={closeAll}>
                                  ...
                                  <ArrowRightIcon className="text-blue-400/40 transition group-hover:translate-x-0.5 group-hover:text-blue-500/75" />
                                </Link>
                              </li>
                            ))}
                          </ul>
            ...
            <div className="hidden lg:block">
              <div
                className={`mb-6 inline-block rounded-full ${BG_INK} px-3 py-1 font-sans text-xs font-semibold uppercase tracking-[0.14em] text-white`}
              >
                Nous contacter
              </div>
              <div className="flex flex-col gap-3 font-display">
                <a
                  ...
                  className={`flex items-center justify-between gap-3 rounded-2xl border border-black/[0.08] bg-white px-4 py-3 no-underline transition-colors hover:border-blue-400/20 hover:bg-slate-50/80 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-offset-2 ${RING_INK_FOCUS}`}
                >
  ...
  return (
    <header className={`${STICKY_POLE_NAV} relative z-50 font-sans text-base font-light`} role="banner">
      {servicesOpen ? (
        <button
          type="button"
      ...
      {mobileOpen ? (
        <div className="fixed inset-0 z-40 bg-black/25 lg:hidden" aria-hidden onClick={closeAll} />
      ) : null}
      ...
      >
          ...
          <div className="flex flex-col gap-1 lg:flex-row lg:items-center lg:gap-1">
            ...
            <div ref={servicesRootRef} className="relative z-[1] w-full lg:w-auto">
              ...
              >
                <span>Services</span>
                <span
                  className={`flex text-current transition-transform duration-200 ${servicesOpen ? "rotate-180" : ""}`}
                >
                  <ChevronDownIcon />
                </span>
              </button>
            ...
            <div ref={resourcesRootRef} className="relative z-[1] w-full lg:w-auto">
              ...
              >
                ...
                <span
                  className={`flex text-current transition-transform duration-200 ${resourcesOpen ? "rotate-180" : ""}`}
                >
                  <ChevronDownIcon />
                </span>
              </button>
                ...
                <div className={`overflow-hidden bg-white ${R_CARD} ${BD_CARD} ${SHADOW_POP}`}>
                  <div className="border-b border-black/[0.06] bg-gradient-to-br from-slate-50/80 to-white px-4 py-3">
                    ...
                    <p className="m-0 mt-1 text-sm leading-snug text-slate-600">
                      Contenus utiles sur nos installations techniques et notre méthode.
                    </p>
                  </div>
                        ...
                        <Link href={item.href} className={resourceRowClass} onClick={closeAll}>
                          <span className="min-w-0 flex-1">
                            <span className="flex items-center justify-between gap-2">
                              ...
                              <ArrowRightIcon className="shrink-0 text-blue-400/45 transition group-hover:translate-x-0.5 group-hover:text-blue-500/80" />
                            </span>
            ...
            <Link href="/maintenance-solutions" className={`${navItem} w-full px-4 lg:w-auto lg:px-3`} onClick={closeAll}>
              Maintenance
            </Link>
            <Link
              href="/contact"
        ...
        <button
          ...
          aria-controls="solutions-nav-mobile"
          onClick={() => setMobileOpen((o) => !o)}
        >
          <span className="flex flex-col gap-1.5">
            <span
              className={`block h-0.5 w-6 rounded-full bg-[#0D0B4A] transition ${mobileOpen ? "translate-y-2 rotate-45" : ""}`}
            />
