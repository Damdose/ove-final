...
export type SolutionsLandingFaqSectionProps = {
  /** `landing` : bloc avec lien vers `/solutions/faq`. `page` : page dédiée, liste seule. */
...
export function SolutionsLandingFaqSection({ variant = "landing" }: SolutionsLandingFaqSectionProps) {
  const baseId = useId();
        ...
        return (
          <div
            ...
            className={`overflow-hidden bg-white ${R_CARD} ${BD_CARD} ${SHADOW_SURFACE} ${SHADOW_SURFACE_HOVER} transition-shadow duration-300 motion-reduce:transition-none ${
              open ? "ring-1 ring-blue-500/22 shadow-[0_8px_32px_-8px_rgba(13,11,74,0.1)]" : ""
            }`}
          >
              ...
              <div className="grid grid-cols-1 items-start gap-10 lg:grid-cols-[minmax(0,0.92fr)_minmax(0,1.08fr)] lg:gap-16 xl:gap-20">
                <div className="lg:sticky lg:top-28">
                  ...
                  <p className={`mt-5 max-w-lg font-sans text-base leading-relaxed sm:text-lg ${TEXT_INK_MUTED}`}>
                    Réponses ciblées sur nos interventions techniques, marques et maintenance. Pour un cas précis,
                    l’équipe Solutions vous répond sur la page contact.
                  </p>
